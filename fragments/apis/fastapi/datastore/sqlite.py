...
@sqlite_router.put("/coins/{ticker}", status_code=200)
async def add_coin(ticker: str, body: CoinBody):
    add_item((ticker.upper(), body.name, body.launched))
    return None


@sqlite_router.delete("/coins/{ticker}", status_code=204)
async def delete_coin_route(ticker: str):
    delete_item(ticker.upper())
    return None


@sqlite_router.patch("/coins/{ticker}", status_code=200)
