...
describe("fastify postgres api", () => {
  after(() => {
    api.close();
  });

  describe("GET /postgres/coins", () => {
  ...
  describe("GET /postgres/coins/:ticker", () => {
    test("should return a specific coin by ticker", async () => {
      const response = await api.inject({
        method: "GET",
        url: "/postgres/coins/BTC",
      });
  ...
  describe("GET /postgres/coins/after/:year", () => {
    test("should return coins launched after a specific year", async () => {
      const response = await api.inject({
        method: "GET",
        url: "/postgres/coins/after/2008",
      });
  ...
  describe("PUT /postgres/coins/:ticker", () => {
    test("creates a new coin", async () => {
      ...
      assert.strictEqual(coin.name, "Postgres test coin");
  ...
  describe("DELETE /postgres/coins/:ticker", () => {
    ...
    test("deleting a non-existent coin is idempotent", async () => {
      const response = await api.inject({
        method: "DELETE",
        url: `/postgres/coins/${randomUUID().slice(0, 6)}`,
      });
      assert.strictEqual(response.statusCode, 204);
    });
  });

  describe("PATCH /postgres/coins/:ticker", () => {
